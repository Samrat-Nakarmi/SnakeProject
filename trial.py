...
#         self.game = Game()
    ...
    #     x = threading.Thread(target=self.snake.voice(), args=(1,), daemon=True)
    #     # y = threading.Thread(target=self.game.run(), args=(1,), daemon=True)
    #     x.start()
    #     # y.start()
...
class Game:
    def __init__(self):
        ...
        # self.thread = Threader(screen)
        # self.thread.thread_deez()

    def resopnd(self, voice_data):
        if 'up' in voice_data:
            self.snake.move_up()
